...
<div class="paper">
	<p><strong><a href="https://doi.org/10.1073/pnas.2502676122" target="_blank" rel="noopener noreferrer">Micro-Macro Analysis of Network Change</a></strong> (<em>Social Networks</em>&nbsp; 2026)</p>
	{% include image.html url="../images/temporal_diagram.png" caption="" height="220px" align=thumbnail %}
	A variety of network models have been developed for longitudinal network data. However, it remains unclear how to analyze cases where the estimand differs from these models' coefficients, e.g., when the model predicts conditional tie probabilities, but the research question concerns change in network segregation. This paper introduces two complementary frameworks. The immediate effect model (IEM) quantifies how the change in micro-level mechanisms between two time points explain the change in macro-level outcomes. The cumulative effect model (CEM) captures the long-term impacts of changes in micro-level mechanisms across multiple time points on certain macro-level outcomes. The models can flexibly accommodate varying numbers of time points, different longitudinal network models, various counterfactual designs, and the existence of time-varying covariates. Their estimands, identifying equations and assumptions, an estimation algorithm, and an empirical example are offered.
